...
with tabs[3]:
    ...
    @st.cache_data(ttl=3600)
    def charger_toutes_sources(pp, tp, ep, ecp):
        dfs = []
        for path, n in [(pp, 5), (tp, 4), (ep, 5), (ecp, 5)]:
    ...
    nb_src = sum(len(pd.read_csv(p)) for p in [pers_rss_path, thema_rss_path, energie_rss_path, economie_rss_path] if os.path.exists(p))
    st.markdown(f"""

...
.person-card {
    background: #0d1520; border: 1px solid #1a2a3a;
    border-radius: 4px; padding: 1rem; margin-bottom: 0.8rem;
}
...
.person-link  {
    ...
    color: #00e5ff; text-decoration: none; border-bottom: 1px solid #00e5ff44;
}
.signal-card {
...
.stTabs [data-baseweb="tab"] {
    ...
    background: transparent; border-radius: 2px; padding: 6px 16px;
}
.stTabs [aria-selected="true"] { background: #1a2a3a !important; color: #00e5ff !important; }

.synthese-card {
    background: #0d1520; border-radius: 4px; padding: 1.4rem;
    margin-bottom: 1.2rem; line-height: 1.75; font-size: 0.88rem; color: #c8d8e8;
}
.synthese-theme {
...
with tabs[0]:
    ...
    st.markdown('<div class="section-header">Évolution</div>', unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    with c1:
        fig = go.Figure()
    ...
    with c2:
        fig2 = px.bar(energie, x="date", y="production_twh", color="source",
    ...
    st.markdown('<div class="section-header">Données — éditable dans data/energie.csv</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
with tabs[1]:
    st.markdown('<div class="section-header">Indicateurs macroéconomiques</div>', unsafe_allow_html=True)
    ...
    for _, row in economie.iterrows():
        st.markdown(f"""
        <div class="signal-card" style="border-color:#1a3a5a">
...
with tabs[3]:
    ...
    if thema_articles.empty:
        ...
        if filtre_lang != "Toutes":
            sources_filtrees = thema_df[thema_df["langue"] == filtre_lang]["nom"].tolist()
            df_t = df_t[df_t["source"].isin(sources_filtrees)]
...
with tabs[4]:
    ...
    if not api_ok:
        st.error("Clé GEMINI_API_KEY introuvable. Va dans Settings → Secrets sur Streamlit Cloud et ajoute : GEMINI_API_KEY = \"ta-clé\"")
        st.stop()
    ...
    @st.cache_data(ttl=3600)
    def charger_toutes_sources(pers_path, thema_path):
        pers   = fetch_rss(pers_path, max_per_source=5)
        thema  = fetch_rss(thema_path, max_per_source=4)
        df = pd.concat([pers, thema], ignore_index=True)
        return df.drop_duplicates(subset=["titre"]).sort_values("date", ascending=False)

    with st.spinner("Chargement des sources…"):
        tous_articles = charger_toutes_sources(pers_rss_path, thema_rss_path)
    ...
    <div style="background:#0d1520; border:1px solid #1a2a3a; border-radius:4px;
                ...
                font-family:Space Mono,monospace;">
        {nb_articles} articles chargés depuis {len(pd.read_csv(pers_rss_path)) + len(pd.read_csv(thema_rss_path))} sources
        · Données énergie : {len(energie)} entrées
        · Données économie : {len(economie)} pays
    </div>""", unsafe_allow_html=True)

    # ── Contexte économie + énergie ───────────────────────────────────────────
    def contexte_energie():
        latest = energie[energie["date"] == energie["date"].max()]
        lignes = [f"- {r['source']} : {r['prix_eur_mwh']} €/MWh, production {r['production_twh']} TWh ({r['note']})"
                  for _, r in latest.iterrows()]
        return "\n".join(lignes)

    def contexte_economie():
        lignes = [f"- {r['pays']} : PIB {r['pib_milliards_usd']} Md$, croissance {r['croissance_pct']}%, inflation {r['inflation_pct']}% ({r['note']})"
                  for _, r in economie.iterrows()]
        return "\n".join(lignes)

    def articles_pour_theme(mots_cles, df, n=12):
        if not mots_cles:
            return df.head(n)
        mask = df["titre"].str.lower().apply(lambda t: any(m in t for m in mots_cles))
        return df[mask].head(n)

    def titres_pour_prompt(df):
   ...
   def appel_gemini(prompt):
    ...
    THEMES = {
        ...
        "🤖 IA":        ["intelligence artificielle", "artificial intelligence", "machine learning", "llm", "gpt", "mistral", "gemini", "openai", "deepmind", "modèle", "neural", "chatgpt"],
        "🚗 Transport": ["transport", "mobilité", "véhicule", "voiture", "aviation", "train", "autonome", "ferroviaire", "fret", "logistique"],
    }
    ...
    if "syntheses_v2" not in st.session_state:
        st.session_state.syntheses_v2 = {}

    if generer:
        st.session_state.syntheses_v2 = {}

        ctx_energie  = contexte_energie()
        ctx_economie = contexte_economie()

        # 1. Résumé global
        with st.spinner("Analyse globale en cours…"):
            tous_titres = titres_pour_prompt(tous_articles.head(30))
...
Articles récents issus de sources de veille innovation :
{tous_titres}
...
Sois analytique et précis. Ne liste pas les articles — fais une vraie synthèse éditoriale."""
            st.session_state.syntheses_v2["global"] = appel_gemini(prompt_global)

        # 2. Synthèses par thème
        for theme, mots_cles in THEMES.items():
            with st.spinner(f"Analyse {theme}…"):
                arts = articles_pour_theme(mots_cles, tous_articles)
                titres = titres_pour_prompt(arts)
                prompt_theme = f"""Tu es un expert en {theme.split(' ')[1]}.

Contexte économique : {ctx_economie}
Contexte énergétique : {ctx_energie}
...
Sois concis et opérationnel."""
                st.session_state.syntheses_v2[theme] = {
                    "texte": appel_gemini(prompt_theme),
                    "nb": len(arts),
                    "articles": arts,
                }

    # ── Affichage des résultats ───────────────────────────────────────────────
    if st.session_state.syntheses_v2:

        # Résumé global
        if "global" in st.session_state.syntheses_v2:
            st.markdown(f"""
            <div class="synthese-card" style="border:1px solid #00e5ff33; border-left:3px solid #00e5ff">
                <div class="synthese-theme" style="color:#00e5ff">◈ Situation globale de l'innovation</div>
                {st.session_state.syntheses_v2['global']}
            </div>""", unsafe_allow_html=True)

        # Thèmes
        col1, col2, col3 = st.columns(3)
        cols_themes = {"⚡ Énergie": col1, "🤖 IA": col2, "🚗 Transport": col3}

        for theme, col in cols_themes.items():
            if theme not in st.session_state.syntheses_v2:
                continue
            data = st.session_state.syntheses_v2[theme]
            c = THEME_COLORS.get(theme, "#6688aa")
            with col:
                st.markdown(f"""
                <div class="synthese-card" style="border:1px solid {c}33; border-top:2px solid {c}; min-height:280px">
